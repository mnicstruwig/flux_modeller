...
class CurveModel:
    ...
    def predict_curves(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        y_hat = self.get_predictions_from_linear_model(
            X_transformed, self.regressor, self.kernel_space
        )

        return self.x_arr, y_hat

    @staticmethod
    def get_predictions_from_linear_model(
        X: np.ndarray, reg: Any, kernel_space: np.ndarray
    ) -> np.ndarray:
